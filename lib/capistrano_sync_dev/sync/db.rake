    ...
    on roles(:db ) do
      ...
      if !File.file?("#{file}.sql") && fetch(:sync_from_file)
        ...
        if fetch(:sync_replace_tables)
          ...
          if table
            query = %x(mysql -u#{ fetch(:sync_to)[ 'username' ] } #{ to_password } -h #{ to_host } #{ fetch(:sync_to)[ 'database' ] } -e 'select max(id) from #{fetch(:sync_table)}')
            @max_id = query.split("\n").last
            max_table_id = @max_id ? "--where='id>#{@max_id}'" : ''
            send(from_local_or_remote, "mysqldump -t --insert-ignore --skip-opt -u #{ fetch(:sync_from)[ 'username' ] } #{ from_password } -h #{ from_host } #{ fetch(:sync_from)[ 'database' ] } #{ fetch(:sync_table) } --single-transaction #{max_table_id} | gzip -9 -c >  #{ file }.sql.gz ")
          else
